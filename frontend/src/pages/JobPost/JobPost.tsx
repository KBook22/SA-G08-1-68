...
import React, { useState, useEffect } from "react"; //  แก้ import
import {
  Form,
  Input,
  Button,
...
const JobPost: React.FC = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        const mapped = res.data.data.map((cat: any) => ({
          id: cat.ID, //  ใช้ id สำหรับส่งกลับ
          category_name: cat.category_name, //  ใช้ name สำหรับแสดงผล
        }));
        console.log("Mapped categories:", mapped);
        setCategories(mapped);
      })
  ...
  // โหลด salarytype จาก backend
  useEffect(() => {
    axios
      .get("http://localhost:8080/api/salarytype")
      .then((res) => {
        const mapped = res.data.data.map((salary: any) => ({
          id: salary.ID, //  ใช้ id สำหรับส่งกลับ
          salary_type_name: salary.salary_type_name, // ใช้ name สำหรับแสดงผล
        }));
        console.log("Mapped salarytype:", mapped);
  ...
  const JobCategorySelector: React.FC = () => (
    ...
    >
      <Select placeholder="เลือกหมวดหมู่">
        {categories.map((cat) => (
          <Select.Option key={cat.id} value={cat.id}>
            {cat.category_name} {/*  แสดงชื่อหมวดหมู่แทนเลข */}
          </Select.Option>
        ))}
      </Select>
    </Form.Item>
    ...
    <div className="jobpost-wrapper">
      <div className="jobpost-card">
        ...
        >
          ...
          {/*  เพิ่ม category + employment type */}
          <JobCategorySelector />
